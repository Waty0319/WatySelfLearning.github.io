...
#예측오차가 M=1 < M=3 로 나타난다. 왜? -> W_0의 값이 계속 더해진다. -> 1~M으로 더하기 -> 반박됨
#원인 : rms의 구조적 결함, t_test과의 차이가 다항함수를 출력하는 반복문안에 들어가서 계속 마이너스되는 구조 -> 해결
#M=9일 때 rms가 더 높게 나타남. 극적인 차이는 아니지만 더 크다는게 핵심.
#정규화항 도입 -> 기존 식을 homogeneous eq로 보고, 정규화항을 포함하여 편미분한 식은 nonhomogeneous eq로 보고 정규화항 없는 식 재활용
# -> A+lamI=T를 푼다.
#규제항 도입 -> 오버피팅 개선 + 기존 최적이라 알려진 M=3보다 성능 좋음 -> 자유도 높이고 규제항 도입하는게
#규제항 없이 자유도만 통제하는 것보다 나을 수 있다.
